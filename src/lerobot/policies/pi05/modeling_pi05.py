...
class PaliGemmaWithExpertModel(
    ...
    def to_bfloat16_for_selected_params(self, precision: Literal["bfloat16", "float32"] = "bfloat16"):
        ...
        # so keep the frozen vision tower consistently bfloat16 and retain only
        # the PaliGemma/Gemma normalization parameters in float32.
    ...
    def embed_image(self, image: torch.Tensor):
        ...
        autocast_enabled = (
            self.train_expert_only and out_dtype == torch.bfloat16 and image.device.type == "cuda"
        )
        with torch.autocast(device_type=image.device.type, dtype=torch.bfloat16, enabled=autocast_enabled):
